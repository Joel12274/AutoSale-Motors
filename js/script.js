// Navegación de la página con el menú hamburguesa en dispositivos móviles
...
navMenu.addEventListener('click', () => {
    navList.style.display = 'flex'; // Mostrar el menú antes de iniciar la animación
    setTimeout(() => {
        navList.classList.add('nav__list--show'); // Iniciar animación
    }, 10); // Pequeño delay para que la animación funcione correctamente
});

// Función para cerrar el menú
navClose.addEventListener('click', () => {
    navList.classList.remove('nav__list--show'); // Quitar animación
    setTimeout(() => {
        navList.style.display = 'none'; // Ocultar después de la animación
    }, 300); // Esperar a que termine la animación antes de ocultarlo
});

navLinks.forEach(link => {
    link.addEventListener('click', () => {
        navList.classList.remove('nav__list--show'); // Inicia la animación de cierre
        setTimeout(() => {
            navList.style.display = 'none'; // Oculta el menú después de la animación
        }, 300);
    });
});
